...
Long answer: Python or Go are nice, but bash is less cumbersome (in my modest opinion) and allows for direct integration of the AWS CLI, without having to use a library and an API. Go is not installed on any system.

### Folder architecture

`/help` contains all you need if you ever are stuck.

`/missions` contains all mission data.
...
Files in each terraform folder are split according to the AWS service they belong to (in the console) (and very uninspiredly, the way I have seen others do it). 
