...
#EC2
resource "aws_instance" "AWS-secgame-mission5-ec2-Evil-server-for-evil-data" {
    ami                         = "ami-0b69ea66ff7391e80"
    availability_zone           = "us-east-1a"
    ebs_optimized               = false
    instance_type               = "t2.micro"
    iam_instance_profile = "${aws_iam_instance_profile.AWS-secgame-mission5-ec2admininstanceprofile.name}"
    monitoring                  = false
	...
	EOF
    tags = {
        Name = "AWS-secgame-mission5-ec2-Evil-server-for-evil-data-${var.id}"
    }
}
...
resource "aws_instance" "AWS-secgame-mission5-ec2-Evil-bastion-for-evil-access" {
	...
	echo "${var.sshprivatekey}" >> /home/ec2-user/AWS-secgame-mission5-keypair-Evilcorp-Evilkeypair-${var.id}.pem
	EOF
    tags = {
