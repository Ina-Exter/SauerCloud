...
if ! aws --profile "$SECGAME_USER_PROFILE" ec2 create-key-pair --key-name "AWS-secgame-mission5-keypair-ddb-handler-$SECGAME_USER_ID" --query 'KeyMaterial' --output text >> resources/ssh_ddb_key.pem
then
...
if ! aws --profile "$SECGAME_USER_PROFILE" ec2 create-key-pair --key-name "AWS-secgame-mission5-keypair-service-$SECGAME_USER_ID" --query 'KeyMaterial' --output text >> resources/ssh_service_key.pem
then
	echo "[AWS-Secgame] Non-zero return code on operation. Abort."
	cd .. || exit
