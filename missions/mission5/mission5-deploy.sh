...
if ! aws --profile "$SAUERCLOUD_USER_PROFILE" ec2 create-key-pair --key-name "SauerCloud-mission5-keypair-ddb-handler-$SAUERCLOUD_USER_ID" --query 'KeyMaterial' --output text >> resources/ssh_ddb_key.pem
then
	...
	mv "mission5-$SAUERCLOUD_USER_ID" ./trash/
	exit 2
fi
sshddbkey=$(<resources/ssh_ddb_key.pem)
chmod 400 resources/ssh_ddb_key.pem
echo "[SauerCloud] Generating service ssh key"
if ! aws --profile "$SAUERCLOUD_USER_PROFILE" ec2 create-key-pair --key-name "SauerCloud-mission5-keypair-service-$SAUERCLOUD_USER_ID" --query 'KeyMaterial' --output text >> resources/ssh_service_key.pem
then
	echo "[SauerCloud] Non-zero return code on operation. Abort."
	cd .. || exit
	...
	mv "mission5-$SAUERCLOUD_USER_ID" ./trash/
	aws --profile "$SAUERCLOUD_USER_PROFILE" ec2 delete-key-pair --key-name "SauerCloud-mission5-keypair-ddb-handler-$SAUERCLOUD_USER_ID"
	exit 2
fi
...
#Get account id
accountID=$(aws --profile "$SAUERCLOUD_USER_PROFILE" sts get-caller-identity --query Account)
...
terraform plan -var="profile=$SAUERCLOUD_USER_PROFILE" -var="id=$SAUERCLOUD_USER_ID" -var="ip=$USER_IP" -var="sshprivatekey=$sshddbkey" -var="sshservicekey=$sshservicekey" -var="accountid=$accountID"
...
then
	...
	mv "./mission5-$SAUERCLOUD_USER_ID" ./trash/
	aws --profile "$SAUERCLOUD_USER_PROFILE" ec2 delete-key-pair --key-name "SauerCloud-mission5-keypair-ddb-handler-$SAUERCLOUD_USER_ID"
	aws --profile "$SAUERCLOUD_USER_PROFILE" ec2 delete-key-pair --key-name "SauerCloud-mission5-keypair-service-$SAUERCLOUD_USER_ID"
	exit 2
fi
...
if ! terraform apply -auto-approve -var="profile=$SAUERCLOUD_USER_PROFILE" -var="id=$SAUERCLOUD_USER_ID" -var="ip=$USER_IP" -var="sshprivatekey=$sshddbkey" -var="sshservicekey=$sshservicekey" -var="accountid=$accountID"
then
	echo "[SauerCloud] Non-zero return code on terraform apply. Rolling back."
	terraform destroy -auto-approve -var="profile=$SAUERCLOUD_USER_PROFILE" -var="id=$SAUERCLOUD_USER_ID" -var="ip=$USER_IP" -var="sshprivatekey=$sshddbkey" -var="sshservicekey=$sshservicekey"
	cd ../../.. || exit
	#If trash doesn't exist, make it
	if [[ ! -d "trash" ]]
	then
...
if ! aws --profile "$SAUERCLOUD_USER_PROFILE" lambda update-function-code --function-name "SauerCloud-mission5-lambda-logs-dump-$SAUERCLOUD_USER_ID" --zip-file fileb://lambda-dump-logs.zip > /dev/null 2>&1
then
...
aws --profile "$SAUERCLOUD_USER_PROFILE" ec2-instance-connect send-ssh-public-key --availability-zone us-east-1a --instance-os-user ec2-user --instance-id "$MAIL_SERVER_INSTANCE_ID" --ssh-public-key file://mailserver_temporary_key.pem.pub > /dev/null 2>&1
...
if ! aws --profile "$SAUERCLOUD_USER_PROFILE" s3 cp aws_key_reminder.txt "s3://sauercloud-mission5-s3-personal-data-emmyselly-$SAUERCLOUD_USER_ID" --quiet
then
...
echo "[SauerCloud] Mission 5 deployment complete. Mission folder is ./mission5-$SAUERCLOUD_USER_ID. Read the briefing to begin, a copy can be found in the mission folder."
...
cd .. || exit
cat "mission5-$SAUERCLOUD_USER_ID/briefing.txt"