#S3
resource "aws_s3_bucket" "SauerCloud-mission5-super-secret-utlimate-s3" {
  bucket        = "sauercloud-mission5-super-secret-utlimate-s3-${var.id}"
  acl           = "private"
  force_destroy = true
  tags = {
    Name = "SauerCloud-mission5-super-secret-utlimate-s3-${var.id}"
  }
}

resource "aws_s3_bucket" "SauerCloud-mission5-s3-es" {
  bucket        = "sauercloud-mission5-s3-personal-data-emmyselly-${var.id}"
  acl           = "private"
  force_destroy = true
  tags = {
    Name = "SauerCloud-mission5-s3-es-${var.id}"
  }
}

#S3 OBJECTS
resource "aws_s3_bucket_object" "picture-of-mila-best-cat-1" {
  bucket = aws_s3_bucket.SauerCloud-mission5-super-secret-utlimate-s3.id
  key    = "mila1.jpg"
  source = "../flags/mila1.jpg"
  tags = {
    Name = "SauerCloud-mission5-super-secret-ultimate-pictures-of-the-developers-cat-${var.id}"
  }
}
...
resource "aws_s3_bucket_object" "flag-hey-there-hacker" {
  ...
  tags = {
    Name = "SauerCloud-mission5-super-secret-ultimate-flag-${var.id}"
  }
}
