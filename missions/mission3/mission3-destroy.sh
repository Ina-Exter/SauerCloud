...
echo "[SauerCloud] Destroy mission3-$SECGAME_USER_ID? (yes/no)"
...
then
	echo "[SauerCloud] Non-zero return code on terraform destroy. There might be a problem. Consider destroying by hand (move to /trash/mission3-$SECGAME_USER_ID/resources/terraform and use terraform destroy, or destroy your resources by hand on the console."
	error_flag=1
fi

#destroy key pair
echo "[SauerCloud] Deleting key pair."
if ! aws --profile "$SECGAME_USER_PROFILE" ec2 delete-key-pair --key-name "SauerCloud-mission3-keypair-Evilcorp-Evilkeypair-$SECGAME_USER_ID"
then
...
then
	echo "[SauerCloud] Non-zero return code on snapshot destruction. Use aws --profile $USER_SECGAME_PROFILE ec2 describe-snapshots and delete-snapshot to manually delete it if needed."
	error_flag=1
fi
