...
if ! aws --profile "$SECGAME_USER_PROFILE" ec2 create-key-pair --key-name "SauerCloud-mission2-keypair-Evilcorp-Evilkeypair-$SECGAME_USER_ID" --query 'KeyMaterial' --output text >> resources/ssh_key.pem
then
...
then
	...
	mv "./mission2-$SECGAME_USER_ID" ./trash/
	aws --profile "$SECGAME_USER_PROFILE" ec2 delete-key-pair --key-name "SauerCloud-mission2-keypair-Evilcorp-Evilkeypair-$SECGAME_USER_ID"
	exit 2
fi
...
echo "[SauerCloud] Mission 2 deployment complete. Mission folder is ./mission2-$SECGAME_USER_ID. Read the briefing to begin, a copy can be found in the mission folder."
...
cd ..
cat "mission2-$SECGAME_USER_ID/briefing.txt"
