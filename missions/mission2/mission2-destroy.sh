...
echo "[SauerCloud] Destroy mission2-$SAUERCLOUD_USER_ID? (yes/no)"
...
then
	...
	cd ../../ || exit
	mv "./trash/mission2-$SAUERCLOUD_USER_ID" ./
	exit 2
fi
...
then
	echo "[SauerCloud] Non-zero return code on terraform destroy. There might be a problem. Consider destroying by hand (move to /trash/mission2-$SAUERCLOUD_USER_ID/resources/terraform and use terraform destroy, or destroy your resources by hand on the console."
	error_flag=1
fi
...
if ! aws --profile "$SAUERCLOUD_USER_PROFILE" ec2 delete-key-pair --key-name "SauerCloud-mission2-keypair-Evilcorp-Evilkeypair-$SAUERCLOUD_USER_ID"
then
