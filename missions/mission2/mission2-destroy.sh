...
if ! aws --profile "$SECGAME_USER_PROFILE" ec2 delete-key-pair --key-name "AWS-secgame-mission2-keypair-Evilcorp-Evilkeypair-$SECGAME_USER_ID"
then
...
if [[ "$error_flag" -eq 1 ]]
then
	echo "[AWS-Secgame] Mission 2 destroy failed somewhere. Error messages should help you fix this."
	exit 1
fi
